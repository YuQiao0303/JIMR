    ...
    for d in range(nd):
        ...
        if len(meshes_gt) > 0:
            ...
            for j in range(len(meshes_gt)):
                ...
                pred = torch.tensor(mesh_pred).cuda().float()
                gt = torch.tensor(meshes_gt[j]).cuda().float()
                # print('---------------pred----------------')
                ...
                proposals_offset=torch.zeros((2)).cuda()
                proposals_offset[1] = pred.shape[0]
                instance_pointnum = torch.tensor(gt[gt==0].shape[0]).unsqueeze(0).cuda().int()
                val = pointgroup_ops.get_iou(pred.cuda().int(), proposals_offset.cuda().int(), gt.long(),
                                              instance_pointnum.int())  # (nProposal, nInstance), float # iou
                val = val[0][0].item()
                # print('val',val)
...
def eval_det_multiprocessing_w_mesh(pred_all, gt_all, thresh=0.25, use_07_metric=True):
    ...
    for scan_id in gt_all.keys():
        ...
        labels, segs, instance_nums  = gt_all[scan_id][0]
        # print('labels',labels) # list of all.
        # print('segs',segs) # list of all.
        # print('instance_nums',instance_nums) # list of all.

        object_num = labels.shape[0]
        for i in range(object_num):
            label = labels[i]
            instance_num = instance_nums[i]
            seg = np.zeros_like(segs).astype(np.int32)
            seg[segs!= i] = -100

            # print('np.unique(seg)',np.unique(seg))
            # print('np.unique(segs)',np.unique(segs))
            # default dict behaviour
            if label not in gt: gt[label] = {}
            if scan_id not in gt[label]: gt[label][scan_id] = []
            # record
            # gt[label][scan_id].append((seg,instance_num))
            gt[label][scan_id].append((seg))
