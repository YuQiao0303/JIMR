...
random_seed_for_sampling_points = 7
IGNORE_CLS = False
EXPORT_ANALYSIS_CSV = True
...
def sample_and_calc_chamfer(pred, gt):
    # pred, gt: trimesh
    pred_points, _ = trimesh.sample.sample_surface(pred, 4096,seed=random_seed_for_sampling_points)
    gt_points, _ = trimesh.sample.sample_surface(gt, 4096,seed=random_seed_for_sampling_points)
...
def eval_det_cls_w_mesh(pred, gt, thresh, use_07_metric):
    ...
    if EXPORT_ANALYSIS_CSV:
        with open('analysis_CD.csv', 'a', newline="") as f2:  # newline = "" delete extra empty lines
            cw = csv.writer(f2)
            for d in range(nd):
                if gt_idx_list[d] < 0:
                    gt_idx = "       "
