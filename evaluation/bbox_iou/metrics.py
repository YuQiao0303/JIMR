import sys
sys.path.append("./") # so that we won't have: ModuleNotFoundError: No module named 'lib'
import os
import numpy as np
import pandas as pd
import torch
...
IGNORE_CLS = False
EXPORT_ANALYSIS_CSV = False
if EXPORT_ANALYSIS_CSV:
    import csv

RFS_labels = ['wall', 'floor', 'cabinet', 'bed', 'chair', 'sofa', 'table', 'door', 'window', 'bookshelf', 'picture',
...
def giou_3d_loss(pred,gt):
    '''
       ...
       :param gt:  [N,7] N bounding boxes or parameters (x,y,z,w,l,h, theta). Assume theta is the angle around z-axis
       :return: the bbox between them
       '''
    results = cal_giou_3d(pred[None, ...], gt[None, ...])
    # iou_loss = results[0][0]
    iou = results[1][0]

    # # results = cal_giou_3d(pred.unsqueeze(0).unsqueeze(0), gt.unsqueeze(0).unsqueeze(0))
    # results = cal_giou_3d(pred.unsqueeze(0)[None, ...], gt.unsqueeze(0)[None, ...])
    # # iou_loss = results[0][0]
...
def eval_det_cls_w_mesh(pred, gt, thresh, use_07_metric):
    ...
    # added by Qiao
    if EXPORT_ANALYSIS_CSV:
        best_val_list = np.ones(nd) * -1
        gt_idx_list = np.ones(nd) * -1
    ...
    for d in range(nd):
        ...
        if len(meshes_gt) > 0:
            ...
            for j in range(len(meshes_gt)):
                ...
                pred = torch.tensor(mesh_pred).cuda().unsqueeze(0).float()
                gt = torch.tensor(meshes_gt[j]).cuda().unsqueeze(0).float()

                # metric is implemented here.
                _, val = cal_giou_3d(pred[None,...], gt[None,...])  # this is ok
                val = val[0][0]

                # val = giou_3d_loss(torch.tensor(mesh_pred).cuda().unsqueeze(0),  # this is not ok
                #                     torch.tensor(meshes_gt[j]).cuda().unsqueeze(0)
                #                     )
                if val > best_val:
                    best_val = val
                    best_idx = j
                    ...
                    # added by Qiao
                    if EXPORT_ANALYSIS_CSV:
                        best_val_list[d] = best_val
                        gt_idx_list[d] = best_idx
            # added by Qiao
            if EXPORT_ANALYSIS_CSV:
                if gt_idx_list[d] < 0:
                    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", best_val)

            print(f'[eval mesh] best bbox iou {best_val}, from {best_idx} in {j}')
    ...
    # added by Qiao to save results for anlaysis
    if EXPORT_ANALYSIS_CSV:
        with open('analysis_bbox_iou.csv', 'a', newline="") as f2:  # newline = "" delete extra empty lines
            cw = csv.writer(f2)
            for d in range(nd):
                if gt_idx_list[d] < 0:
                    gt_idx = "       "
                    ...
                    gt_idx = gt_idx_list[d]
                    best_val = best_val_list[d]
                # semantic	scan_id	 n_pred	 n_gt	pred_idx	gt_idx	 scores	bbox_iou	tp	 fp	tp_cumsum	fp_cumsum	precision	recall
                cw.writerow([
                    '', scan_ids[d], nd, len(all_stats[scan_ids[d]]['meshes_gt']),
                    d, gt_idx, -sorted_scores[d], best_val, tp_mesh_list[d], fp_mesh_list[d],
                    tp_mesh_cumsum[d], fp_mesh_cumsum[d], prec_mesh[d], rec_mesh[d]
                ])
...
def eval_det_multiprocessing_w_mesh(pred_all, gt_all, thresh=0.25, use_07_metric=True):
    ...
    for scan_id in pred_all.keys():
        ...
        # print(type(pred_all[scan_id]))
        # print(len(pred_all[scan_id]))
        # print(type(pred_all[scan_id][0]))
        if len(pred_all[scan_id])==0:
            pass
        else:
    ...
    for scan_id in gt_all.keys():
        ...
        labels, bboxes = gt_all[scan_id][0]
        object_num = bboxes.shape[0]
        for i in range(object_num):
            label = labels[i]
            bbox = bboxes[i]
    ...
    for label in gt.keys():
        ...
        # here we print label added by Qiao
        if EXPORT_ANALYSIS_CSV:
            with open('analysis_bbox_iou.csv', 'a', newline="") as f2:  # newline = "" delete extra empty lines
                cw = csv.writer(f2)
                cw.writerow([
                    CAD_labels[int(label)]
                ])
    ...
    for i, label in enumerate(gt.keys()):
        if label in pred:
            ...
            (rec_mesh[label], prec_mesh[label], ap_mesh[label]), (PQ_mesh[label], SQ_mesh[label], RQ_mesh[label]) = (0,
                                                                                                                     0,
                                                                                                                     0), (
                                                                                                                    0,
                                                                                                                    0,
...
class APCalculator(object):
    ...
    def compute_metrics(self):
        ...
        for key in sorted(ap_mesh.keys()):
            key = int(key)# don't know why but need this, it was float
        ...
        for key in sorted(PQ_mesh.keys()):
            key = int(key)  # don't know why but need this
            clsname = self.class2type_map[key] if self.class2type_map else str(key)
