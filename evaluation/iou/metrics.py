...
def eval_det_cls_w_mesh(pred, gt, ovthresh=0.25, use_07_metric=False, get_iou_func=get_iou, get_iou_mesh=compute_mesh_iou):
    ...
    for d in range(nd):
        ...
        if BBGT.size > 0:
            ...
            for j in range(BBGT.shape[0]):
                ...
                if iou_mesh > ovmax_mesh:
                    ...
                    # added by Qiao
                    if EXPORT_ANALYSIS_CSV:
                        best_val_list[d] = ovmax_mesh
                        gt_idx_list[d] = jmax_mesh
        ...
        if ovmax_mesh > ovthresh:
            ...
            fp_mesh[d] = 1.
    if EXPORT_ANALYSIS_CSV:
        fp_mesh_list = fp_mesh
        tp_mesh_list = tp_mesh
    # for bbox
    fp = np.cumsum(fp)
    tp = np.cumsum(tp)
    ...
    if EXPORT_ANALYSIS_CSV:
        fp_mesh_cumsum = fp_mesh
        tp_mesh_cumsum = tp_mesh
    ...
    if EXPORT_ANALYSIS_CSV:
        with open('analysis_mesh_iou.csv', 'a', newline="") as f2:  # newline = "" delete extra empty lines
            cw = csv.writer(f2)
            for d in range(nd):
                if gt_idx_list[d] < 0:
                    gt_idx = "       "
                ...
                cw.writerow([
                    '', scan_ids[d], nd,
                    npos,#len(all_stats[scan_ids[d]]['meshes_gt']),
                    d,
                    gt_idx,
                    -sorted_scores[d],
                    best_val,
    ...
    def compute_metrics(self):
        ...
        if EXPORT_ANALYSIS_CSV:
            with open('analysis_mesh_iou.csv', 'a', newline="") as f2:  # newline = "" delete extra empty lines
                ...
                cw.writerow([
                    'semantic', 'scan_id', 'n_pred', 'n_gt', 'pred_idx', 'gt_idx',
                    'scores', 'iou', 'tp', 'fp',
                    'tp_cumsum', 'fp_cumsum', 'precision', 'recall'
